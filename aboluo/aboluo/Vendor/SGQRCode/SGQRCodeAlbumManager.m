...
- (void)enterImagePickerController {
    ...
    //[imagePicker.navigationBar  setBackgroundImage:[UIImage imageNamed:@"navbar5"] forBarMetrics:UIBarMetricsDefault];
    imagePicker.navigationBar.barTintColor = MainThemeColor;
    //设置右侧取消按钮的字体颜色
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    if (features.count == 0) {
        ...
        [self.currentVC dismissViewControllerAnimated:YES completion:^{
            if (self.delegate && [self.delegate respondsToSelector:@selector(QRCodeAlbumManager:didFinishPickingMediaWithResultWithError:)]) {
                [self.delegate QRCodeAlbumManager:self didFinishPickingMediaWithResultWithError:@"没有识别到二维码"];
            }
        }];
        return;
