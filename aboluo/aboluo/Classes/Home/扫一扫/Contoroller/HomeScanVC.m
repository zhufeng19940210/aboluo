...
#import "HomeScanVC.h"
#import "SGQRCode.h"
@interface HomeScanVC ()<SGQRCodeScanManagerDelegate, SGQRCodeAlbumManagerDelegate>
...
- (void)setupNavigationBar {
    ...
    [self setRightButtonText:@"相册" withColor:[UIColor whiteColor]];
...
-(void)QRCodeAlbumManager:(SGQRCodeAlbumManager *)albumManager didFinishPickingMediaWithResultWithError:(NSString *)error{
    [SVProgressHUD showErrorWithStatus:@"扫描有误"];
    [self.navigationController popViewControllerAnimated:YES];
}
- (void)QRCodeAlbumManager:(SGQRCodeAlbumManager *)albumManager didFinishPickingMediaWithResult:(NSString *)result {
    if ([self.delegate respondsToSelector:@selector(getScanStr:)]) {
        [self.delegate getScanStr:result];
    }
    [self.navigationController popViewControllerAnimated:YES];
}
...
- (void)QRCodeScanManager:(SGQRCodeScanManager *)scanManager didOutputMetadataObjects:(NSArray *)metadataObjects {
    ...
    if (metadataObjects != nil && metadataObjects.count > 0) {
        ...
        NSString *qrCode = [obj stringValue];
        if ([self.delegate respondsToSelector:@selector(getScanStr:)]) {
            [self.delegate getScanStr:qrCode];
        }
        [self.navigationController popViewControllerAnimated:YES];
    } else {
        [SVProgressHUD showErrorWithStatus:@"扫描有误"];
...
- (UILabel *)promptLabel {
    if (!_promptLabel) {
        ...
        _promptLabel.text = @"请对准二维码/条码,耐心等待";
    }
    return _promptLabel;
}
- (UIView *)bottomView {
...
- (UIButton *)flashlightBtn {
    if (!_flashlightBtn) {
        ...
        [_flashlightBtn setBackgroundImage:[UIImage imageNamed:@"SGQRCodeFlashlightOpenImag"] forState:(UIControlStateSelected)];
